...
export class OrderService {
  ...
  async findById(id: string, user: AuthUser): Promise<OrderEntity> {
    const order = await this.getById(id);
    await this.assertOrderAccess(order, user);
    return order;
  }

  private async getById(id: string): Promise<OrderEntity> {
  ...
  async assertOwnedByUser(
    ...
    const order = await this.getById(id);
    await this.assertOrderAccess(order, user, allowPrivileged);
  }
  ...
  private async assertOrderAccess(
    ...
    if (allowPrivileged && (user.role === 'OPERATOR' || user.role === 'ADMIN')) {
      return;
    }
    if (!order.billId) {
   ...
   */
  async create(billId: string, amount: number, user: AuthUser): Promise<OrderEntity> {
  ...
  async submitPayment(id: string, user: AuthUser): Promise<OrderEntity> {
    return this.transition(id, user, 'PENDING_PAYMENT');
  }
   ...
   */
  async confirmPaid(id: string, user: AuthUser): Promise<OrderEntity> {
    if (user.role !== 'OPERATOR' && user.role !== 'ADMIN') {
      throw new ForbiddenError('无权确认支付成功');
    }
  ...
  /** 完成订单：PAID → COMPLETED。 */
  async complete(id: string, user: AuthUser): Promise<OrderEntity> {
    return this.transition(id, user, 'COMPLETED');
  }

  /** 关闭订单（超时/取消）：CREATED 或 PENDING_PAYMENT → CLOSED。 */
  async close(id: string, user: AuthUser): Promise<OrderEntity> {
    return this.transition(id, user, 'CLOSED');
  }
